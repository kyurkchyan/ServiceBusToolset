        ...
        for (var batchStart = 0; batchStart < specs.Count; batchStart += BatchSize)
        {
            ...
            for (var attempt = 0; attempt < maxAttempts && pending.Count > 0; attempt++)
            {
                ...
                foreach (var msg in received)
                {
                    ...
                    if (diagnosticIdByMessageId.TryGetValue(msg.MessageId, out var diagnosticId))
                    {
                        var propertiesToModify = new Dictionary<string, object>
                        {
                            ["Diagnostic-Id"] = diagnosticId
                        };
                        await receiver.DeadLetterMessageAsync(msg, propertiesToModify, reason,
                            "Generated by TestHarness for testing", cancellationToken);
                    }
                    else
                    {
