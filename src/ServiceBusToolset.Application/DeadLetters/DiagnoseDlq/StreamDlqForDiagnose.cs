...
public sealed record StreamDlqForDiagnoseCommand(string FullyQualifiedNamespace,
                                                 EntityTarget Target,
                                                 bool MergeSimilar = false) : ICommand<Result<DlqScanSession>>;

public sealed class StreamDlqForDiagnoseCommandHandler(IServiceBusClientFactory clientFactory)
    : ICommandHandler<StreamDlqForDiagnoseCommand, Result<DlqScanSession>>
{
    public ValueTask<Result<DlqScanSession>> Handle(
        StreamDlqForDiagnoseCommand command,
