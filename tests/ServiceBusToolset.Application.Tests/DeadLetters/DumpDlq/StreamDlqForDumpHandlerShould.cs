...
public class StreamDlqForDumpHandlerShould
{
    private readonly MockServiceBusClientFactory _mockFactory;
    private readonly StreamDlqForDumpCommandHandler _handler;

    public StreamDlqForDumpHandlerShould()
    {
        _mockFactory = MockServiceBusClientFactory.Create();
        _handler = new StreamDlqForDumpCommandHandler(_mockFactory.Object);
    }
    ...
    public async Task ReturnSession_WhenHandled()
    {
        ...
        var command = new StreamDlqForDumpCommand("test.servicebus.windows.net",
