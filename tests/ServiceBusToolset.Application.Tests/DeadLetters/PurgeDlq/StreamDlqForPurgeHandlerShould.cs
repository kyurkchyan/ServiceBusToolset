...
public class StreamDlqForPurgeHandlerShould
{
    private readonly MockServiceBusClientFactory _mockFactory;
    private readonly StreamDlqForPurgeCommandHandler _handler;

    public StreamDlqForPurgeHandlerShould()
    {
        _mockFactory = MockServiceBusClientFactory.Create();
        _handler = new StreamDlqForPurgeCommandHandler(_mockFactory.Object);
    }
    ...
    public async Task ReturnSession_WhenHandled()
    {
        ...
        var command = new StreamDlqForPurgeCommand("test.servicebus.windows.net",
